const { Contract, BrowserProvider, JsonRpcProvider, Wallet } = require("ethers");
const fs = require("fs");
...
async function test() {
  ...
  console.log(typeof registry.uploadTrack);
  const tx = await registry.uploadTrack("test", "test", "test", "test", "test");
